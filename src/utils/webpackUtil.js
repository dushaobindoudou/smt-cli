...
const webpackUtil = {
    // 获取模块详情
    getModuleInfos(clientConfInfo, smtConf) {
        const sourcePath = path.join(smtConf.smtClientDir, clientConfInfo.sourceDir);
        const webpackConfPath = util.getWebpackConfDir(smtConf);
        const modules = {};
        try {
    ...
    // 构建没有js 入口的页面
    execBuildNoEntryPage(moduleInfos, smtConfInfo) {
        const watchFiles = [];
    ...
    // 获取webpack配置信息
    getWebpackConfInfo(smtConf, moduleName, pageName) {
        if (!moduleName) {
            return null;
        }
        // 优先获取 自定义配置，如果没有找到自定义配置 则使用默认配置
        const wpCustomDir = util.getWebpackConfDir(smtConf);
        let relName = '';
        if (moduleName) {
            relName += moduleName;
        }
    ...
    // 构建 widget
    execBuildWidget(moduleInfos, smtConfInfo) {
        const watchFiles = [];
        if (!Array.isArray(moduleInfos.noEntry)) {
            return null;
        }
    ...
    // 构建有js入口的页面
    execBuildPage(webpackConfigs, bundleInfo, smtConfInfo, onEnd) {
        ...
        const configModule = this.getWebpackConfInfo(smtConfInfo,
    ...
    findTpl(smtConf, options, clientInfo) {
        const entrys = {
            entry: [],
            noEntry: [],
        };
        ...
        const opts = Object.assign({}, {
            ...
            incss: false,
            moduleDir: '',
            srcDir: path.join(smtConf.smtClientDir, clientInfo.sourceDir), // 项目的源代码目录
            clientNodeModules: path.join(smtConf.smtClientDir, './node_modules'),
        ...
        this.findPage(opts.finalPath, (res) => {
            let jsEntryName = '';
            let jsPath = '';
            const relRoot = res.tpl.replace(smtConf.smtClientDir + path.sep, '');
            ...
            if (res.hasJS) {
                jsPath = res.js.replace(smtConf.smtClientDir, '');
    ...
    findWidet(smtConf, options, clientInfo) {
        const entrys = {
            entry: [],
            noEntry: [],
        };
        ...
        this.findPage(opts.widgetPath, (widget) => {
            const relRoot = widget.tpl.replace(smtConf.smtClientDir, '');
            const srcRepStr = clientInfo.sourceDir.replace(/\W/ig, '') + path.sep;
            const finalConfig = {
                viewPath: relRoot.replace(srcRepStr, clientInfo.module.viewsDir),
                watch: opts.isWatch,
    ...
    requireModule(moduleName, smtConfInfo) {
        if (!module) {
            return null;
        }
        try {
            // project module
            const clientModule = path.join(smtConfInfo.smtClientDir, './node_modules');
